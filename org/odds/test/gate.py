...
from org.odds.test.realm import Realm, DEFAULT_DELIMITER
...
def main():
    realm = Realm()
    user1 = RealmUser(realm)

    # share public keys and make sure they are signed and verified before usage
    user1.keyring.load(realm.key.pubkey)
    realm.keyring.load(user1.key.pubkey)
    ...
    payload = None
    with user1.keyring.key(realm.id) as enc_1_key:  # get user's public key for realm sending
        payload = {
    ...
    (i.e hash([orignal message]:[random token]:[timestamp])). this will be signed
    ...
    if payload is not None:
        with realm.keyring.key(realm.id) as dec_1_key:
            dec1 = dec_1_key.decrypt(PGPMessage.from_blob(payload['message']))
            msg1 = dec1.message
            with realm.keyring.key(user1.id) as sig_check_pub:
    ...
    if answered_door is True and msg1 is not None:
        ...
        with realm.keyring.key(realm.id) as sig_2_key:
    ...
    if token_payload is not None:
        ...
        with user1.keyring.key(user1.id) as user1_priv_key:
            ...
            with user1.keyring.key(realm.id) as token_check_pub:
